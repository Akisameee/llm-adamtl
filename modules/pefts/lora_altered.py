...
class Lora_Linear_Altered(Base_Adapter):
    ...
    def get_grads(self, grad_dict: dict):

        if self.name is None:
            lora_A_key = list(filter(lambda k: k.endswith('lora_A.weight'), grad_dict.keys()))[0]
            lora_B_key = list(filter(lambda k: k.endswith('lora_B.weight'), grad_dict.keys()))[0]
        else:
            lora_A_key = self.name + '.lora_A.weight'
            lora_B_key = self.name + '.lora_B.weight'
