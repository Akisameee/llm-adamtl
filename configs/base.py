...
@dataclass
class Instruct_Dataset_Config(object):
...
@dataclass
class HumanFeedback_Dataset_Config(object):
...
@dataclass
class Accelertor_Config(object):
...
def get_dataclass_fields(
    ...
    for field in dataclasses.fields(dataclass):
        ...
        if dataclasses.is_dataclass(field.type):
            sub_fields_out = get_dataclass_fields(
                field.type,
                field.default,
                prefix = name_new
            )
            branches.append({
                'name': field.name,
                'name_new': name_new,
                'dataclass': sub_fields_out[0]['dataclass'],
                'sub': sub_fields_out[0]['sub']
            })
            fields += sub_fields_out[1]
        else:
            branches.append({
                'name': field.name,
                'name_new': name_new,
...
def parse_args_into_dataclass(
    ...
    dataclass = branches['dataclass']
    args = {}
    for arg in branches['sub']:
        if arg['dataclass'] is not None:
            parse_args_into_dataclass(arg, args_namespace)
        else:
            args[arg['name']] = args_namespace[arg['name_new']]
            setattr(dataclass, arg['name'], args_namespace[arg['name_new']])
